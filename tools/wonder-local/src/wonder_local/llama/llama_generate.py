...
def llama_generate(self, *args):
    ...
    default_model_path = (
        Path.home() / "Models" / "tinyllama" / "tinyllama-1.1b-chat-v1.0.Q8_0.gguf"
    )
    ...
    self.logger.info(
        f"[cyan]Running llama.cpp inference with model:[/cyan] {model_path}"
    )
    ...
    try:
        ...
        if not hasattr(self, "llama") or self.llama is None:
            self.llama = Llama(
                model_path=str(model_path), n_ctx=512, n_threads=6, verbose=False
            )
